...
const GameBoard = () => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const headBeforeMove = { x: snake.segments[0].x, y: snake.segments[0].y };
      ...
      if (head.x === bonus.x && head.y === bonus.y) {
        snake.grow();  // You'll implement this
        bonus = generateBonus(field);  // Respawn bonus
      }
      render();
    };
    ...
    function generateBonus(field: GameField): Bonus {
      ...
      return new Bonus(bonusX, bonusY, '/assets/bonuses/banana.png');
    }
